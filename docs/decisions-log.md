...
## 2026-07-04 — Пакет UI-правок владельца (полировка)

- Приоритеты и статусы задач с эмодзи: ⚪ Обычный, ⭐ Важный, 🔥 Срочный; 🕓 Не начато, ⚡ В процессе, ⏳ Ожидает (селект списка — 📥 Входящие).
- Все поля ввода получили видимые контуры — единый класс `.field`: рамка, мягкая заливка, акцентное фокус-кольцо. В правой панели задач: название, статус, «чего ждём», описание, добавление подзадачи. Аналогично в быстрых заметках, привычках, быстром добавлении.
- Новый общий компонент выбора даты/времени `DatePicker` (вместо нативных `input type=date/time`): быстрые кнопки (Сегодня/Завтра/Через неделю/Без даты), мини-календарь с навигацией по месяцам, поле времени с очисткой и кнопкой «Готово». Используется в быстром добавлении и редакторе задачи.
- Топбар менее заметный: без фона, блюра и нижней границы; поиск — приглушённая плашка без рамки.
- Сайдбар: кастомные тултипы модулей (тёмная плашка, появление с задержкой 0.3s, лёгкий сдвиг) через `data-tip` вместо нативных `title`.
- Требование целостности: новые модули обязаны использовать `.field` для полей ввода и `DatePicker` для дат (зафиксировано в agent-handoff.md).
- `docs/agent-handoff.md` полностью переписан под текущий проект: правила работы с владельцем, workflow, структура кода, статус модулей, команды Tauri, миграции, дизайн-система, Windows-грабли, roadmap — чтобы любой новый агент мог продолжить работу без потери контекста.

## 2026-07-04 — AGENTS.md

- В корень репозитория добавлен `AGENTS.md` — первый файл для любого агента: правила стиля общения (русский язык, живой тон с эмодзи, структурные ответы, итог со ссылкой на коммит, пачечная работа без лишних вопросов, фиксация решений владельца) + короткая сводка проекта и порядок чтения документации.

## 2026-07-04 — Топбар без поиска, HTML-превью, «Сегодня» во всю ширину

- Строка поиска убрана из топбара (владелец). Палитра осталась на Ctrl+K, в топбаре теперь только дата.
- Модуль «Сегодня» — во всю ширину: снято ограничение `max-width: 980px`.
- Добавлен статичный HTML-вариант приложения `docs/design/app-preview.html`. Правило (владелец): при каждом изменении UI обновлять этот файл в том же коммите, поддерживая его актуальным. Зафиксировано в `AGENTS.md`.

## 2026-07-04 — «Сегодня»-таймлайн, свой ПКМ, темы v2, Проекты + Дневник

- «Сегодня» переработан в **таймлайн дня** по референсу владельца: две колонки (08:00–16:00 и 16:00–23:59) с вертикальной линией, карточками задач (фиолетовая полоса) и привычек (зелёная), меткой «Сейчас HH:MM». Задачи и привычки без времени — чипы над таймлайном.
- У привычек появилось поле «время» (`habits.time_of_day`, миграция 0005) — для размещения на таймлайне.
- Системное контекстное меню вебвью заглушено; добавлено **своё контекстное меню** в стиле приложения (компонент ContextMenu). Подключено: карточки таймлайна, чипы, строки задач, привычки (удаление), проекты (архивирование).
- Форма добавления привычки переработана: поле `.field`, пресеты дней (Каждый день / Будни / Выходные), пилюли дней, необязательное время.
- Темы: теперь **8 штук** (+ Графит, Океан, Роза). Выбор в настройках минималистичный — цветовые точки (фон + акцент), без карточек-превью (решение владельца).
- Новые модули: **Проекты** (по docs/modules/projects.md: компактный список слева с тихим плюсом, редактируемые поля Описание/Этап/Что мешает/Следующие действия/Заметки, автосохранение, архивирование по ПКМ) и **Дневник** (по docs/modules/diary.md: месячная сетка на всю высоту, оценка дня ?/10 с цветом от красного к зелёному, модалка дня: оценка 0–10, настроение, энергия, «Что сделано» с автоподтяжкой задач/привычек + ручные пункты с точкой, свободная запись). Медиа-вложения дневника отложены до общей стратегии хранения файлов.
- Миграция 0005: `habits.time_of_day`, таблицы `projects`, `diary_entries`.
